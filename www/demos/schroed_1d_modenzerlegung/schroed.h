...
static inline float complex eulerpot_plus(float complex links, float complex mitte, float complex rechts, float pot_alt, float pot_neu, float zeitbruchteil, float laplace_gewichtung){
    return (laplace_gewichtung *(links + rechts - mitte * 2) - mitte * (pot_alt + (pot_neu-pot_alt)*zeitbruchteil)) * I ;
}
