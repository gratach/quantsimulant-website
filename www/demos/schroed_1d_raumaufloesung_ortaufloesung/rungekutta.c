...
{
	int i;
	void** tempvektoren = zmalloc(sizeof(tempvektoren)*ordnung, muell);
	for(i = 0; i<ordnung; i++)
		tempvektoren[i] = zmalloc(sizeofvector , muell);
	argf_var(rueck, 
		rungekutta, arbeiter, muell, rungekuttadetails, zustand_alt, zustand_neu, ordnung, tempvektoren, sizeofvector, aenderung,  adiereauf, rungematrix, zeitschritt);
	return rueck;
}
